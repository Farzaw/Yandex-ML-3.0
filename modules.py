...
Original file is located at
    https://colab.research.google.com/drive/1aircTDiDNDxNtljWVXCg8mKDbvZKvMcZ
...
class SoftMax(Module):
    ...
    def updateOutput(self, input):
        input_normalized = input - input.max(axis=1, keepdims=True)
        exp_values = np.exp(input_normalized)
    ...
    def updateGradInput(self, input, gradOutput):
        S = self.output
        batch_sum = np.sum(gradOutput * S, axis=1, keepdims=True)
        self.gradInput = S * (gradOutput - batch_sum)
        return self.gradInput
...
class LogSoftMax(Module):
    ...
    def updateOutput(self, input):
        ...
        sum_exp = np.sum(np.exp(self.output), axis=1, keepdims=True)
    ...
    def updateGradInput(self, input, gradOutput):
        ...
        batch_sum = np.sum(gradOutput, axis=1, keepdims=True)
        self.gradInput = gradOutput - S * batch_sum
...
class BatchNormalization(Module):
    ...
    def updateGradInput(self, input, gradOutput):
        if self.training:
            ...
            term1 = gradOutput * inv_std
            term2 = inv_std / N * np.sum(gradOutput, axis=0, keepdims=True)
            sum_dy_x = np.sum(gradOutput * x_centered, axis=0, keepdims=True)
            term3 = x_centered * (inv_std ** 3) / N * sum_dy_x

            self.gradInput = term1 - term2 - term3
        else:
            inv_std = 1.0 / np.sqrt(self.moving_variance + self.EPS)
            self.gradInput = gradOutput * inv_std
        return self.gradInput
...
class Dropout(Module):
    ...
    def updateOutput(self, input):
        if self.training:
            self.mask = (np.random.rand(*input.shape) < (1 - self.p)).astype(np.float32)
...
class ELU(Module):
    ...
    def updateGradInput(self, input, gradOutput):
        grad_mask = np.where(input > 0, 1, self.output + self.alpha)
...
class SoftPlus(Module):
    ...
    def updateOutput(self, input):
        ...
        self.output = np.maximum(input, 0) + np.log1p(np.exp(-abs_input))
        return self.output

    def updateGradInput(self, input, gradOutput):
        sigmoid = 1.0 - np.exp(-self.output)
...
class Conv2d(Module):
            ...
            for o in range(out_channels):
                for i in range(in_channels):
                    self.output[b, o, :, :] += scipy.signal.correlate2d(
                        input_padded[b, i, :, :], self.W[o, i, :, :], mode='valid'
                    )
    ...
    def updateGradInput(self, input, gradOutput):
        ...
        for b in range(batch_size):
            for i in range(in_channels):
                for o in range(out_channels):
                    self.gradInput[b, i, :, :] += scipy.signal.convolve2d(
                        gradOutput[b, o, :, :], self.W[o, i, :, :], mode='same'
                    )
    ...
    def accGradParameters(self, input, gradOutput):
        ...
        for o in range(out_channels):
            for i in range(in_channels):
                for b in range(batch_size):
                    self.gradW[o, i, :, :] += scipy.signal.correlate2d(
                        input_padded[b, i, :, :], gradOutput[b, o, :, :], mode='valid'
                    )
            self.gradb[o] = np.sum(gradOutput[:, o, :, :])
...
class MaxPool2d(Module):
    ...
    def updateOutput(self, input):
        ...
        h_pool, w_pool = input_h // k, input_w // k

        reshaped = input.reshape(n, c, h_pool, k, w_pool, k)

        swapped = reshaped.swapaxes(3, 4)

        self.output = swapped.max(axis=(4, 5))
        max_values = self.output[..., np.newaxis, np.newaxis]
        self.max_mask = (swapped == max_values)
    ...
    def updateGradInput(self, input, gradOutput):
        n, c, h_pool, w_pool = gradOutput.shape
        k = self.kernel_size

        grad_expanded = gradOutput[..., np.newaxis, np.newaxis]
        grad_broadcast = grad_expanded * self.max_mask.astype(gradOutput.dtype)

        grad_swapped = grad_broadcast.swapaxes(3, 4)
        self.gradInput = grad_swapped.reshape(self.input_shape)
